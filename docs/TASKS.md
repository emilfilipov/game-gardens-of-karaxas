...
| GOK-MMO-049 | ✅ | 2 | Stabilize Epic D/E rollout by fixing backend startup crash in observability metrics state initialization, migrating JWT handling to `PyJWT` (removing vulnerable `python-jose/ecdsa` path), and enforcing non-root backend container runtime for CI security compliance. |
