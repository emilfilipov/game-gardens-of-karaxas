...
| Task ID | Status | Complexity | Detailed Description |
| --- | --- | --- | --- |
| COI-2D-053 | ✅ | 3 | Added `shadow_graph` pass 06 with a zoomed-node interaction model: only the selected graph node is visible, each node is large enough to host its full menu payload, route chips on edges represent clickable navigation targets, black/white high-fidelity shadow-blob styling is applied, and every non-boot node includes a `Back` action explicitly mapped to previous-node traversal. |
| COI-2D-052 | ✅ | 3 | Extended `tools/generate_shadow_graph_concepts.py` and produced `concept_art/shadow_graph/pass_05` to attach explicit functional menu payloads to graph-focused nodes (auth, register, play empty, play selected, create, system/MFA, update), addressing the gap where prior shadow-graph outputs showed blobs/routes without readable menu surfaces. |
| COI-2D-051 | ✅ | 4 | Added strict UI concept iteration policy to `AGENTS.md` and started a new graph-native concept family under `concept_art/shadow_graph` via `tools/generate_shadow_graph_concepts.py`, executing true sequential passes (`pass_01` shadow baseline, `pass_02` blood-palette readability pass, `pass_03` reduced-label + camera-travel affordance refinement) with per-pass process notes. |
| COI-2D-050 | ✅ | 4 | Produced a fresh 20-pass outside-the-box concept exploration branch under `concept_art/option_outsidebox_20pass` (generated via `tools/generate_ui_concept_outsidebox_20.py`) with per-pass outputs for gateway/lobby/create/system/update surfaces, solid-color background direction, custom-drawn motif elements, and icon-first navigation experiments while preserving functional parity for login/register/optional-MFA/settings/create/select/update. |
| COI-2D-049 | ✅ | 4 | Created a new 10-pass concept exploration branch under `concept_art/option_nova_10pass` using a strict sequential generate-review-improve loop, with broader IA experimentation (auth+update fusion, stage/floating/hybrid workspace modes, icon-first rail variant, and COI emblem branding replacing title headline) while preserving functional parity surfaces for auth/register/MFA/update notes/play/create/system. |
