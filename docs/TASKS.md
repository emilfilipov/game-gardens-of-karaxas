...
| AOP-PIVOT-037 | ✅ | 2 | AOP-PIVOT-036 | Added one-command local PoC bootstrap runner (`scripts/run_local_poc_stack.sh`) that starts backend + world-service with readiness gates, executes deterministic account/character seed (`backend/scripts/seed_local_poc_account.py`), emits startup handoff payload for `client-app`, runs backend vertical-slice smoke, and documents first-run/reset runbooks. |
| AOP-PIVOT-038 | ✅ | 4 | AOP-PIVOT-037 | Added authenticated world sync endpoint (`POST /gameplay/world-sync`) that advances deterministic ticks and returns travel/logistics/trade/espionage/politics/battle snapshots, extended backend observability with world-sync success/failure/latency metrics, and wired `client-app` bootstrap shell polling with deterministic tick application, reconnect backoff, stale-data indicators, and live map/panel data hydration. |
| AOP-PIVOT-039 | ✅ | 4 | AOP-PIVOT-038 | Added first playable real-time battle scene in `client-app` with fixed-step visual time, front-line/unit overlays, command HUD controls, backend battle command routing (`/gameplay/battle/start`, `/gameplay/battle/command`), and deterministic client/backend regression coverage for battle command/response projection. |
| AOP-PIVOT-040 | ✅ | 3 | AOP-PIVOT-039 | Replaced remaining placeholder panel content with live authoritative world-sync hydration (`character`/`household`/domain states), added panel action dispatch endpoint (`POST /gameplay/domain-action`) with optimistic in-flight guards + clear success/failure feedback, and extended backend/client tests for action mapping and payload validation. |
| AOP-PIVOT-041 | ✅ | 3 | AOP-PIVOT-040 | Added Windows installer acceptance smoke suite (`scripts/windows_installer_acceptance_smoke.ps1`) and release-pipeline gates that verify game/designer install->update flow plus gameplay/handoff regression tests, with per-run smoke summary artifact upload in `.github/workflows/release.yml`. |
| AOP-PIVOT-042 | ✅ | 2 | AOP-PIVOT-041 | Added dated external PoC release gate evidence bundle under `docs/release-gates/2026-03-13-single-player-poc/`, pointer manifest `docs/release-gates/current_gate.json`, and automated validator `backend/scripts/validate_external_poc_release_gate.py` wired into the release workflow. |
