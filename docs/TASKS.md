...
| GOK-MMO-224 | ⬜ | 4 | Formalize and implement 8-direction character animation contract (`N`,`NE`,`E`,`SE`,`S`,`SW`,`W`,`NW`) for all core actions (idle/walk/run/attack/cast/hit/death), including importer validation and missing-direction CI checks. |
| GOK-MMO-225 | ⬜ | 4 | Implement modular character visual stack (paperdoll layering): base body + slot overlays (`head`, `chest`, `legs`, `hands`, `feet`, `weapon_main`, `weapon_off`) so equipped items change in-game appearance without client rebuild. |
| GOK-MMO-226 | ⬜ | 3 | Extend DB-driven item/content schemas to include per-slot visual definitions (atlas key, draw layer, pivot offsets, tint channels, optional effect hooks) and bind equipment state to runtime render composition. |
| GOK-MMO-227 | ⬜ | 3 | Add editor + validation workflow for equipment visuals: slot compatibility checks, per-direction sprite completeness, fallback/default visuals, and side-by-side preview of saber vs longsword and other slot variants. |
| GOK-MMO-228 | ⏳ | 1 | Blocked input ticket for production art pass: provide (1) style reference set (2-4 examples), (2) target sprite resolution + zoom, (3) v1 equipment slot list, (4) v1 weapon/item visual list (at least saber + longsword variants), and (5) v1 per-action frame-count contract; point-2 guidance to evaluate: recommended MMO baseline is high-detail character frames (`128x128`) + tighter logical iso footprint (`64x32`) + slightly zoomed-out default camera (`~0.80x`, suggested range `0.70x-1.10x`) so more world is visible while keeping placement precision; future change-cost matrix: camera zoom/view distance/fog-of-war are low-cost, sprite on-screen size/render-scale policy are medium-cost, core tile footprint and pivot conventions are high-cost migration items. |
