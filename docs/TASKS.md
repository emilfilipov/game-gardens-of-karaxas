...
| GOK-MMO-226 | ⬜ | 3 | Extend DB-driven item/content schemas to include per-slot visual definitions (atlas key, draw layer, pivot offsets, tint channels, optional effect hooks, style tags) and enforce V1 equip conflicts/handedness: two-handed weapons disable `weapon_off`, `full_body` armor disables `pants`, shields are off-hand only, one-handed weapons default main hand but may be assigned to either hand when no conflict. |
