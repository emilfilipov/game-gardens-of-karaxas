...
| GOK-MMO-190 | ⬜ | 4 | Build Godot-native `Level Editor v2` shell with modern docked layout (asset browser, hierarchy/layers panel, inspector, viewport, validation panel, change queue) sized for full-screen production workflows. |
...
| GOK-MMO-203 | ⬜ | 3 | Add in-editor playtest mode in Godot that spawns selected character profile and simulates runtime movement/collision/transition behavior without full relaunch, including quick return to edit mode. |
