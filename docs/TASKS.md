...
| --- | --- | --- | --- |
| GOK-MMO-163 | ⬜ | 4 | Define and implement tower-zone runtime partitioning model: each level/zone is an independently addressable world segment with local render layers, explicit `order_index` in the tower sequence, and transition graph links (`stairs`, `ladder`, `elevator`) to destination zones/spawn markers. Include migration and API shape updates so zone identity/order is decoupled from render-layer numbering. |
| GOK-MMO-164 | ⬜ | 4 | Implement seamless no-loading-screen streaming strategy: client always renders current zone only, preloads adjacent linked zone data/assets when player enters transition proximity volumes, and performs hot swap on transition trigger without blocking UI. Add runtime cache policy (active + warm neighbors), eviction rules, and failure fallback messaging if a target zone cannot be streamed in time. |
| GOK-MMO-165 | ⬜ | 3 | Add backend/player-state spawn rules for zone entry: new characters spawn at tower-first level (lowest `order_index`) at that level's spawn marker; returning characters spawn at persisted last location (`level_id`, `location_x`, `location_y`). Ensure auth->play handoff bypasses account cards and enters gameplay scene directly once a character is chosen for login/play flow. |
| GOK-MMO-166 | ⬜ | 3 | Extend Level Editor metadata with descriptive naming and ordering fields: keep existing technical level name/key, add `descriptive_name` input, and expose current tower order context. Validate uniqueness/length constraints and include this metadata in staged draft payloads and publish flow. |
| GOK-MMO-167 | ⬜ | 4 | Add transition-link editing in Level Editor: place/configure transition assets (`stairs`, `ladder`, `elevator`) with destination zone + destination spawn marker binding. Include placement tools, per-link property editor, delete/edit interactions, and serialized persistence through Save Local/Publish Changes paths. |
| GOK-MMO-168 | ⬜ | 3 | Build admin `Level Order` management UI: card list of all levels with drag-and-drop reordering, inline descriptive name display, unsaved change indicator, and staged local draft + publish workflow parity with other admin tools. Persist reordered `order_index` values atomically and audit publish actions. |
| GOK-MMO-169 | ⬜ | 3 | Add transition asset support and palette integration: provide placeholder/themed assets/icons for stairs/ladder/elevator in editor and runtime renderer, include tooltip descriptions, and ensure layer-safe placement defaults. Prepare asset schema so richer art variants can be swapped later without changing logic. |
| GOK-MMO-170 | ⬜ | 3 | Refactor in-game menu scope to minimal live gameplay controls only: cogwheel menu while in game must contain `Settings`, `Logout Character`, `Logout Account`, `Exit Game` and remove non-gameplay entries from in-game context. Ensure each action persists location/session state correctly before scene/account transitions. |
