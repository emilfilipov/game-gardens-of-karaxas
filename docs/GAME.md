...
- Loop: launcher opens on a fixed-size window (`980x560`) with anchored top metadata/account bar, central news/patch panes, and bottom update/play bar; after successful login it auto-refreshes latest version/news/patch notes every 60 seconds from backend release metadata, exposes account actions through a top-right menu (including logout when gameplay is not launched), and when `Play` is pressed it re-checks latest version and runs silent background update with staged progress states (`Checking Latest Version`, `Downloading`, `Installing Update`, `Starting Game`) in a full-height rectangular progress lane aligned with `Play`, without a separate authenticated status text row, before launching fullscreen with authenticated handoff and minimizing while staying active without opening a companion terminal window; logout clears local auth state even if backend revocation fails, then returns to login with an error notice.
...
#### Cost guardrails baseline
- Purpose: keep PoC development financially sustainable while systems mature.
