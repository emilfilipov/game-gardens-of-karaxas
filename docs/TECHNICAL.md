...
- Self-hosted release workflow resolves Python 3.11 via existing `python`/`py` launcher and temporary command shims (instead of `actions/setup-python`) to avoid execution-policy blocks on service-account hosted runners.
