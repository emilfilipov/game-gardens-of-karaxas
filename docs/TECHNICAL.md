...
- Self-hosted release workflow now explicitly activates MSVC toolchain environment (`ilammy/msvc-dev-cmd`) before Rust build steps so `link.exe` is discoverable under service-account runner contexts.
