...
- New exploratory family `concept_art/shadow_graph` is generated through `tools/generate_shadow_graph_concepts.py` using strict single-pass iteration. Current passes (`pass_01..pass_05`) cover logo-node boot, reveal-on-click graph expansion, camera pan focus, explicit login-gated flow (`logo -> auth -> play_hub`), and focused node payload panels for auth/register/play-empty/play-selected/create/system(MFA)/update so graph navigation and menu functionality are shown together.
