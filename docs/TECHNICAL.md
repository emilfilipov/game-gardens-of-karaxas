...
- Auth/session continuity gate now sets `PYTHONPATH` to an absolute backend path at runtime and installs test deps through `python -m pip` to keep pytest imports stable on self-hosted Windows service runners.
