...
from app.api.routes.gameplay import run_vertical_slice_loop  # noqa: E402
...
from app.schemas.gameplay import VerticalSliceLoopRequest  # noqa: E402
...
def _seed_user_character(db: Session) -> tuple[User, UserSession, Character]:
    user = User(email="vertical@test.com", display_name="Vertical", password_hash="hash", is_admin=False)
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_vertical_slice_loop_persists_character_writeback(monkeypatch: pytest.MonkeyPatch) -> None:
    db = _db_session()
    user, session, character = _seed_user_character(db)
    command_calls: list[dict] = []

    def _dispatch(*, trace_id: str, command: dict) -> dict:
        command_calls.append({"trace_id": trace_id, "command": command})
        return {"accepted": True}

    monkeypatch.setattr(gameplay_routes, "dispatch_control_command", _dispatch)
    monkeypatch.setattr(gameplay_routes, "advance_ticks", lambda *, now_ms: {"current_tick": 52})
    monkeypatch.setattr(
        gameplay_routes,
        "fetch_battle_state",
        lambda: {
    ...
    response = run_vertical_slice_loop(
        VerticalSliceLoopRequest(character_id=character.id, campaign_destination_settlement_id=222),
        context=_auth_context(user, session),
        db=db,
    )

    assert response.accepted is True
    assert response.reason_code == "battle_writeback_persisted"
    assert response.world_commands_queued == 5
    assert len(command_calls) == 5
    assert response.campaign_tick == 52
    assert response.character_level >= 1
    assert response.persisted_location_x == 222
    assert response.persisted_location_y == 222

    refreshed = db.get(Character, character.id)
    assert refreshed is not None
    assert refreshed.location_x == 222
    assert refreshed.location_y == 222
    assert refreshed.experience == response.character_experience


def test_vertical_slice_loop_returns_502_when_world_service_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    db = _db_session()
    user, session, character = _seed_user_character(db)
    ...
    with pytest.raises(HTTPException) as exc:
        run_vertical_slice_loop(
            VerticalSliceLoopRequest(character_id=character.id),
            context=_auth_context(user, session),
            db=db,
        )
